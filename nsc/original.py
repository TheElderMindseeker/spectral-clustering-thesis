...
def VoxelNystromSC(P,gap,s_file,d_file,bwidth):
    zd=6
    a=np.array([1,1,zd])
    P=P/a
    nP=np.shape(P)[0]
    ...
    csv_write1=csv.writer(s_file,dialect='excel')
    csv_write1.writerow(('x','y','z','label'))
    for i in range(np.shape(PP0)[0]):
        csv_write1.writerow((PP0[i,0],PP0[i,1],PP0[i,2],int(PP0[i,3])))

    csv_write2=csv.writer(d_file,dialect='excel')
    csv_write2.writerow(('TreeID', 'TreeTopX', 'TreeTopY', 'Height', 'CrownRadius'))
    for i in range(np.shape(SSbio)[0]):
        csv_write2.writerow((int(SSbio[i,0]),SSbio[i,1],SSbio[i,2],SSbio[i,4],SSbio[i,3]))
